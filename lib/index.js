...
var unpackBundle = function(options) {
	...
	return pcall(tmp.dir, tmp, {
		...
		var params = [ path.resolve( options.bundle ) ];
		spawn('unzip', params, {
			cwd: builddir
			...
			if (exitCode === 0) {
				...
				deferred.reject(new Error(
					'unzip '+params.join(' ')+' exited with code '+exitCode
				));
			}
...
var processImages = function(metabook, builddir, options) {
	...
	return imagedb.forEach(function(key, val) {
		...
		var pp = when.resolve().then(function() {
			...
			return (safe === filename) ? path.join(builddir, 'images', safe) :
				renameFile(path.join(builddir, 'images'), filename, safe);
