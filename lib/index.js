...
var generateLatex = function(metabook, builddir, imagemap, options) {
	...
	var head = STD_HEADER, columns = metabook.columns, toc = metabook.toc; 
        var logoSrc = metabook.logoSrc, footerLeft = metabook.footerLeft, footerMiddle = metabook.footerMiddle;
        if (columns === 1) {
	...
	head = head.replace(/\]\{article\}/, ',' + metabook.papersize + 'paper$&');
        ...
        head = head.replace(/\\ampmtime/, '\\footnotesize ' +  footerMiddle);
	// Book or article?
	var hasChapters =
