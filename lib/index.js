...
*/
	...
	'\\lstset{',
     ...
     /* NEXT RELEASE - TODO: 
     	- fix footer into first page (see lines about 1960)
     	- try to move footer text from index.js to localSettings, so not hardcoded
     //Set footer*/
     ...
     '\\fancypagestyle{plain}{',
        ...
        '\\fancyfoot[LE,LO]{LeftFooterText}',
        '\\fancyfoot[CE,CO]{MiddleFooterText}',
...
var generateLatex = function(metabook, builddir, imagemap, options) {
	...
	var head = STD_HEADER, columns = metabook.columns, toc = metabook.toc; 
        var logoSrc = metabook.logo, footerLeft = metabook.footerLeft, footerMiddle = metabook.footerMiddle;
        if (columns === 1) {
	...
	head = head.replace(/\]\{article\}/, ',' + metabook.papersize + 'paper$&');
        //Replace default footer text with it provided from LocalSettings
        head = head.replace(/LeftFooterText/, footerLeft);
        head = head.replace(/MiddleFooterText/, footerMiddle);
	// Book or article?
	var hasChapters =
