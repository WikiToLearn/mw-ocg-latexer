...
// `oldname` and `newbase`).
var renameFile = P.guard(P.guard.n(1), function(dir, oldname, newbase) {
...
var convertGif = P.guard(P.guard.n(5), function(info) {
	return P.call(tmp.tmpName, tmp, {
		dir: info.imagedir,
...
// Takes a {imagedir,filename}, returns a promise for an
// {imagedir,filename} with the converted filename.
var convertSvg = P.guard(P.guard.n(5), function(info) {
	return P.call(tmp.tmpName, tmp, {
		dir: info.imagedir,
...
// Takes a {imagedir,filename}, returns a promise for an
// {imagedir,filename} with the converted filename.
var convertJpg = P.guard(P.guard.n(5), function(info) {
...
var processImages = function(metabook, builddir, options) {
	...
	return imagedb.forEach(function(key, val) {
		...
		var pp = Promise.resolve({ imagedir: imagedir, filename: filename });
