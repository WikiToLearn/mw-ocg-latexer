...
var NOTOFONT =  'Path=' +
	...
	'Extension=.ttf';
var COMPUTERMODERN = 'Path=' +
	path.join(__dirname, '..', 'fonts', 'cm-unicode-0.7.0') + '/,' +
	'Extension=.ttf,' + 'BoldFont=cmunbx, ItalicFont=cmunsl';
...
var SCRIPT_FONTS = {
	...
	//Local copy, from fonts/cm-unicode-0.7.0
	'default': {
		name: 'cmunrm',
		opts: COMPUTERMODERN,
	},
...
var STD_HEADER = [
	...
	'\\documentclass[11pt,twocolumn,oneside,fleqn]{article}',
	'\\pagestyle{headings}',
	'\\usepackage{fontspec, xunicode, polyglossia, graphicx, xltxtra, anyfontsize}',
	...
	'\\definecolor{wtl_green}{HTML}{00613A}',
	'\\usepackage[unicode,colorlinks,breaklinks,urlcolor=wtl_green,linkcolor=black]{hyperref}',
	'\\urlstyle{same}',
	// Narrower margins.
...
Formatter.prototype.begin = function(env, opts, extra) {
	this.envBreak();
	var stack = this.saveInlineStack();
	this._addDecoration({
		type: 'start-block',
		value: '\\begin{' + env + '}' + (opts ? ('[' + opts + ']') : '') + (extra ?  extra : ""),
	});
	this.restoreInlineStack(stack);
	this.envBreak();
	this.resetSOL();
...
Visitor.prototype.visit = function(node) { //document.body
...
Visitor.prototype.visitA = function(node) {
	...
	if (href && !this.inHeading && !node.querySelector('img')) {
		if (/^#/.test(href)) {
			...
			return '\\footnote{' + this.wrap('\\href{' + href + '}', node) + '}';
		}
	}
	this.visitChildren(node);
};
...
Visitor.prototype.visitTABLE = function(node) {
	...
	if (this.inFloat) { return; } // XXX tables should be permitted in captions
	this.inFloat = true;
	var envName = (this.options.columns === 2) ? 'table*' : 'table';
	var fullWidth = true; // XXX
	this.format.begin(envName, 'tbh!');
	this.format.writeDecorated('\\centering');


	// count maximum number of columns.
	// XXX look at <colgroup>?
	var tbody = childSelector(node, 'tbody');
	var rows = childSelectorAll(tbody, 'tr');
	var maxCols = 1; // at least one column
	rows.forEach(function(tr) {
		var cols = childSelectorAll(tr, 'th,td').reduce(function(s, td) {
			return s + ((+td.getAttribute('colspan')) || 1);
		}, 0);
		maxCols = Math.max(maxCols, cols);
	});
	var width = (1/maxCols) + (fullWidth ? '\\textwidth' : '\\columnwidth');
	var colfmt = '*{' + maxCols + '}{p{' + width + '}}';
	this.format.begin('tabular', [ '{' + colfmt + '}'], "{" + "|l".repeat(maxCols) + "|}" );

	var sawTH = false;
	rows.forEach(function(tr) {
		// separate rows containing <th> from rows not containing <th>
		//var hasTH = !!childSelector(tr, 'th');
		/*if (sawTH && !hasTH) {
			this.format.writeDecorated('\\hline');
			this.format.envBreak();
			this.format.resetSOL();
		}*/
		//sawTH = hasTH;
		...
		// emit contents of <td>/<th>
		childSelectorAll(tr, 'th,td').forEach(function(td, idx) {
			if (idx !== 0) { this.format.envBreak(); this.format.writeDecorated('&'); }
			this.format.envBreak();
			this.format.resetSOL();
			// XXX handle COLSPAN, ROWSPAN
			if (td.matches('th')) {
				this.format.writeDecorated('\\cellcolor{lightgray}');
				visitINLINE.call(this, td, 'B');
			} else {
				this.visitChildren(td);
			}
		}, this);
		this.format.writeDecorated('\\tabularnewline');
		this.format.envBreak();
		this.format.resetSOL();
	}, this);
	...
	this.format.end('tabular');

	/*var caption = childSelector(node, 'caption');
	if (caption) {
		var curPoly = Polyglossia.lookup(this.currentLanguage);
		var oldContextDir = this.format.contextDir;
		if (this.currentLanguage !== this.tocLanguage) {
		...
		// appearance hack: strip out initial boldface
		// (often the entire caption is bolded)
		var capNode = DomUtil.first_child(caption);
		if (capNode && capNode.nodeType === node.ELEMENT_NODE &&
			capNode.matches('b:not([lang]):not([dir])')) {
			this.visitChildren(capNode);
			capNode = capNode.nextSibling;
			for ( ; capNode; capNode = capNode.nextSibling) {
				this.visit(capNode);
			}
		} else {
...
Visitor.prototype.visitFIGURE = function(node, extraCaption) {
	...
	// Inline image?
        //Inline images are ignored --> all figures
	/*if (isInline) {
		...
		console.warn("##########VISIT FIGURE FILENAME############");
		console.warn("filename:" + filename);
		this.format.writeDecorated('{\\centering\\includegraphics[width=0.70\\textwidth]{' + filename + '}}');
		return;
        }*/
	this.inFloat = true;
	// Floats seem to revert to collectionLanguage.
	this.format.begin('figure', 'H');
...
Visitor.prototype['visitTYPEOF=mw:Extension/source'] = function(node) {
	...
	if(lang){
		...
		this.format.begin('lstlisting','language=');
		this.format.writeDecorated(content);
		this.format.end('lstlisting');
	}
};
...
Visitor.prototype.visitNOWIKI = function(node){
	...
	his.format.begin('lstlisting','numbers=none');
	IN_VERBATIM = true;
	this.visitChildren(node);
	this.format.end('lstlisting')
	IN_VERBATIM= false;
...
Visitor.prototype.visitPRE = function(node){
	console.warn('TAG PRE');
	this.format.begin('lstlisting','numbers=none');
	IN_VERBATIM = true;
	this.visitChildren(node);
	this.format.end('lstlisting')
	IN_VERBATIM= false;
...
Visitor.prototype['visitTYPEOF=mw:Extension/math'] = function(node, display) {
	...
	if (mr.euro_required || mr.teubner_required) {
		if (EXTRA_WARNINGS) {
			console.warn('Math markup requires additional packages');
		}
		return;
	}
	var displayType = JSON.parse(node.getAttribute('data-mw')).attrs.display;
...
var updateMetabook = function(metabook, options) {
	...
	metabook.toc = toc;
        ...
        if (metabook.footer && /^(yes|no)$/.test(metabook.footer)) {
		footer = (metabook.footer === 'yes');
	}
	if (options.footer === true || options.footer === false) {
		footer = options.footer; // Command-line override.
	}
	metabook.footer = footer;
...
var generateLatex = function(metabook, builddir, imagemap, options) {
	...
	var head = STD_HEADER, columns = metabook.columns, toc = metabook.toc, footer = metabook.footer;
        //var footerLeft = metabook.footerLeft, footerMiddle = metabook.footerMiddle, footerRight = metabook.footerRight;
        if (columns === 1) {
	...
	if(footer) {
			var logoFooter = path.join(__dirname, '..', 'tex', 'logoFooter.tex');
            head = head.replace(/(\\pagestyle\{headings\})/,
            	"\\usepackage{lastpage}\n\\usepackage{fancyhdr}\n\\usepackage{etoolbox}"
            	+ "\n\\fancypagestyle{plain}{\n\\fancyhf{}\n\\fancyhfoffset{30pt}\n\\lhead{\\nouppercase{\\leftmark}}\n\\rhead{\\thepage\\ / \\pageref{LastPage}}"
            	+ "\n\\cfoot{\\input{" + logoFooter + "}}"
            ...
            /*if(footerLeft !== ""){
                head = head.replace(/\\today/, '\\input{' + logoPath + '}');
            }
            if(footerMiddle !== ""){
                head = head.replace(/\\currenttime/, '\\footnotesize ' +  footerLeft);
            }
            if(footerRight !== ""){
                head = head.replace(/\\thepage\\ \/ \\pageref\{LastPage\}/, footerRight);
            }*/
	...
	if (!singleItem) {
		...
		var logoNoTitle = path.join(__dirname, '..', 'tex', 'logoNoTitle.tex');
		var license = path.join(__dirname, '..', 'tex', 'license.tex');
		var barra = path.join(__dirname, '..', 'tex', 'barra.tex');
	}
	...
	// Start the doc!
	headFormat.writeDecorated('\\begin{document}');
	if(singleItem)
		headFormat.writeDecorated('\\maketitle\n');
	else{
		headFormat.writeDecorated('\n\\newgeometry{top=7cm,bottom=2.5cm,left=2.5cm,right=2.5cm,heightrounded,bindingoffset=5mm}\n\\begin{titlepage}\n\\centering\n{\\fontsize{30}{40}\\selectfont \\textbf{');
		headFormat.dirBreak();
		if(collectionTitle){
			headFormat.write(collectionTitle);
		}else {
			headFormat.writeDecorated('WikiToLearn');
		}
		headFormat.writeDecorated('}}');
		headFormat.writeDecorated('\\\\\n\\bigskip\n\\bigskip\n\\input{' + barra + '}');
		if (metabook.subtitle) {
	...
	if (toc) {
		headFormat.writeDecorated('\\tableofcontents\n');
	...
	return P.forEachSeq(metabook.items || [], function(item) {
		...
		// xelatex bug described here: http://tex.stackexchange.com/questions/276742/order-of-words-disturbed-by-using-polyglossia-with-rtl-language
		// incompatibility problem among polyglossia and amstext-xetex-bidi.def
		// which is part of the bidi bundle
		// the solution is to manually set \@Latintrue in case of rtl language
		if (clPoly.dir === 'ltr') {
			s += '\\makeatletter\n' ;
			s += '\\appto{\\blockextras@' + clPoly.lang + '}{\\@ifundefined{if@Latin}{}{\\@Latintrue}}\n' ;
			s += '\\appto{\\inlineextras@' + clPoly.lang + '}{\\@ifundefined{if@Latin}{}{\\@Latintrue}}\n' ;
			s += '\\makeatother' ;
		}
