...
Visitor.prototype['visitLANG='] = function(node) {
	...
	var isBlock = /^(BLOCKQUOTE|BODY|CENTER|DIV|DL|FIGURE|H[1-6]|OL|P|TABLE|UL)$/.test(node.nodeName); // XXX others?
...
var generateLatex = function(metabook, builddir, imagemap, options) {
	...
	var usedLanguages = new Set(); // usedLanguages doesn't include default lang
	head += '\n\\input{'+path.join(builddir, 'languages.tex')+'}'; // we'll put used languages here
