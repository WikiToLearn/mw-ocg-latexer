...
var LANGUAGE_FONTS = {
	'urdu': { name: 'Nafees', opts: FAKESTYLES } // from fonts-nafees
};
...
var renameFile = guard(guard.n(1), function(dir, oldname, newbase) {
	return P.call(fs.exists, fs, path.join(dir, newbase)).then(function(exists){
		if (!exists) {
...
var generateLatex = function(metabook, builddir, imagemap, options) {
	...
	return P.forEachSeq(metabook.items, function(item) {
		...
		// hackity hack: initialize the LTRfont for the collection language
		updateLTRfont({
