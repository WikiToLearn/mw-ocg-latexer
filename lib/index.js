...
var STD_HEADER = [
	...
	//'\PassOptionsToPackage{english}{babel}', ALE
	'\\pagestyle{headings}',
	...
	'\\definecolor{wtl_green}{HTML}{52A552}',
	'\\usepackage[unicode,colorlinks,breaklinks,allcolors=wtl_green,linkcolor=wtl_green]{hyperref}',
	'\\urlstyle{same}',
	// Narrower margins.
	'\\usepackage[a4paper,top=3cm,bottom=3cm,left=3.5cm,right=3.5cm,heightrounded,bindingoffset=5mm]{geometry}',
	'\\usepackage{floatrow}',
	'\\floatsetup[figure]{capposition=beside,capbesideposition={bottom, outside}}',
	'\\captionsetup{indention=-4.5cm}',
	'\\usepackage{lastpage}',
	//'\\usepackage[english]{babel}', ALE
...
*/
	...
	'\\lstset{',
     ...
     //Set footer is commentendddhudeujf
...
Visitor.prototype.visit = function(node) { //document.body
	...
	console.log("Visiting " + name + " " + type + " " + node.text);
...
var generateLatex = function(metabook, builddir, imagemap, options) {
	...
	if(footer) {
            ...
            head = head.replace(/(\\pagestyle\{headings\})/, 
            	...
            	+ "{\\thispagestyle{empty}\\@starttoc}\n{}{}"
            	//+ "\n\\patchcmd{\\headrule}{\\hrule}{\\color{grigio}\\hrule}{}{}\n\\patchcmd{\\footrule}{\\hrule}{\\color{grigio}\\hrule}{}{}"
            	+ "\n\\makeatother");
	...
	if(singleItem)
		...
		headFormat.writeDecorated(' }}');
		headFormat.writeDecorated('\\bigskip\n\\bigskip\n\\input{' + barra + '}');
		if (metabook.subtitle) {
	...
	write.article = function(item) {
		...
		console.log("SONOQUI: " + item.type + " " + item.title)
...
var convert = function(options) {
	...
	console.log("Converting with these options: ");
	console.log(options);
