...
var STD_HEADER = [
	...
	'\\documentclass[11pt,twocolumn,twoside,fleqn]{article}',
	'\\pagestyle{headings}',
...
var processImages = function(metabook, builddir, options) {
	...
	return imagedb.forEach(function(key, val) {
		...
		console.warn("########PROCESS IMAGES FILENAME AND KEY NAME IN IMAGEDB###########");
		console.warn("filename:" + filename);
		...
		console.warn("########IMAGES FILENAME AND KEY IN PP PROMISE(Record the final filename)###########");
		console.warn("filename:" + info.filename);
		console.warn("key:" + key);
...
var generateLatex = function(metabook, builddir, imagemap, options) {
	...
	if(footer) {
            ...
            head = head.replace(/(\\pagestyle\{headings\})/, 
            	...
            	+ "\n\\fancyfoot[CE,CO]{\\input{" + logoFooter + "}}"
            	+ "\n\\renewcommand{\\footrulewidth}{0.5pt}\n\\renewcommand{\\headrulewidth}{0.5pt}\n}\n\\pagestyle{plain}"
	...
	var emitTitle = function(title) {
		...
		if(singleItem){
			...
			titleFormat.writeDecorated('\\title{{\\fontsize{50}{60}\\selectfont ');
			titleFormat.dirBreak();
			titleFormat.write(title);
			titleFormat.writeDecorated('}');
			titleFormat.lineBreak();
			if (metabook.subtitle) {
				titleFormat.writeDecorated('{\\fontsize{30}{40}\\selectfont ');
				titleFormat.dirBreak();
				titleFormat.write(metabook.subtitle);
				titleFormat.writeDecorated('}');
				titleFormat.lineBreak();
			}
			titleFormat.writeDecorated('\\input{' + logoCover + '}');
			titleFormat.dirBreak();
			titleFormat.writeDecorated('}');
			titleFormat.envBreak();
		}
